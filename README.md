...
1. **敏感信息检查**：
   - [x] **代码文件**：已检查，无硬编码的 API 密钥或密码。所有密钥均通过环境变量加载。
   - [x] **配置文件**：`config.yaml` 中仅包含配置项名称，无实际密钥值。
   - [x] **个人信息**：
     - **注意**：如果您在项目中包含其他个人文件，请确保在上传前进行检查。
   - [x] **数据文件**：`data/` 目录下的 CSV/JSON 仅包含生成的图像元数据，不包含用户隐私数据。

2. **安全建议**：
   - 不要将包含真实 API 密钥的 `.env` 文件提交到版本控制系统（建议在 `.gitignore` 中添加 `.env`）。
   - 定期轮换 API 密钥。
